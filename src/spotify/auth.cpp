...
auth_t auth_cache::authenticate(const httplib::Params &params)
{
    ...
    // any error while authorization means critical error
    if (!http::is_success(res))
        throw std::runtime_error(std::format("Authorization error: {}", http::get_status_message(res)));

    // TODO: an error generated by invalid object parsing is not clear enough: what request it was? what was the obejct? log is empty as well
    json::parse_to("{111}", result);
