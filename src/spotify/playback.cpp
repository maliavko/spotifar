...
void playback_cache::on_data_synced(const playback_state &data, const playback_state &prev_data)
{
    if (data.item != prev_data.item)
        ObserverManager::notify(&playback_observer::on_track_changed, data.item);

    if (data.progress_ms != prev_data.progress_ms)
        ObserverManager::notify(&playback_observer::on_track_progress_changed,
                                data.item.duration, data.progress);

    if (data.device.volume_percent != prev_data.device.volume_percent)
        ObserverManager::notify(&playback_observer::on_volume_changed,
                                data.device.volume_percent);

    if (data.shuffle_state != prev_data.shuffle_state)
        ObserverManager::notify(&playback_observer::on_shuffle_state_changed,
                                data.shuffle_state);

    if (data.repeat_state != prev_data.repeat_state)
        ObserverManager::notify(&playback_observer::on_repeat_state_changed,
                                data.repeat_state);

    if (data.is_playing != prev_data.is_playing)
        ObserverManager::notify(&playback_observer::on_state_changed,
                                data.is_playing);

    if (data.context != prev_data.context)
        ObserverManager::notify(&playback_observer::on_context_changed, data.context);

    if (data.actions != prev_data.actions)
        ObserverManager::notify(&playback_observer::on_permissions_changed, data.actions);
